...
MYSQL_CONFIG = {
    ...
    "password": os.getenv("MYSQL_PASSWORD", "1234"),    # 改为自己的 mysql数据库 密码！
...
def get_text_aiPersonality():
    """用于存放AI的身份和行为准则"""
    # 不存在则创建时写入默认文本，存在则直接跳过
    script_dir = Path(__file__).parent / 'AI_PERSONALITY.txt'
    file = script_dir
    if not file.exists():
        file.write_text('你是一个可爱活泼,爱用颜文字的小朋友,名叫小可,是一名很厉害很幽默的全栈工程师', encoding='utf-8')
...
AI_PERSONALITY = get_text_aiPersonality()
...
# ==================== 自动打开浏览器 ====================
# 如果 8000端口 被占用，可以改成 5000 或 5050 
def open_browser():
    time.sleep(1.5)
